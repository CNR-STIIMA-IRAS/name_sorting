                       ...
                       const std::string& whoami)
{
  if (names.size()<order_names.size())
  {
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "[ %s ]The vector of names to be sorted has size %zu, that is smaller than vector of the sorted names (%zu)", whoami.c_str(),names.size(),order_names.size());
    return false;
  }
  if (names.size()!=position.size())
  {
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "[ %s ]Input Mismatch. The size of the vector of names is %zu, while position size is %zu", whoami.c_str(),names.size(),position.size());
    return false;
  }
  if (names.size()!=velocity.size())
  {
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "[ %s ]Input Mismatch. The size of the vector of names is %zu, while velocity size is %zu", whoami.c_str(),names.size(),velocity.size());
    return false;
  }
  if (names.size()!=effort.size())
  {
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "[ %s ]Inut Mismatch. The size of the vecotr of the names is %zu, while effort size is %zu", whoami.c_str(),names.size(),effort.size());
    return false;
  }
  ...
  for (unsigned int iOrder=0;iOrder<order_names.size();iOrder++)
  {
    if (names.at(iOrder).compare(order_names.at(iOrder)))
    {
      for (unsigned int iNames=iOrder+1;iNames<names.size();iNames++)
      {
        ...
        {
          std::stringstream ss;
          RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "[ %s ]The Joint '%s' that is in the vector of the sorted names, is missing in the vector to be sorted.", whoami.c_str(),order_names.at(iOrder).c_str());
          ss << "Sorted Names: <";
          for( size_t i=0;i<order_names.size();i++)
            ss << order_names.at(i) <<",";
          ...
          for( size_t i=0;i<names.size();i++)
            ss << names.at(i) <<",";
          ss <<">";
          RCLCPP_ERROR_STREAM(rclcpp::get_logger("rclcpp"),  ss.str() );
          return false;
        }
      }
    }
